...
router.get("/", authenticate, async (req,res)=>{
    try{
        ...
        res.status(500).json({
            errorMessage:
            "sorry something went wrong getting users"
        })
...
router.get("/:username", authenticate, async (req,res)=>{
    try{
        ...
        res.status(500).json({
            errorMessage: 
            "Sorry something went wrong getting user"
        })
        throw new Error(error)
    }
})
...
router.get("/:username/favorited", authenticate, async (req,res)=>{
    try{
        ...
        res.status(500).json({
            errorMessage:
            "Sorry something went wrong getting user's favorited recipes"
        })
        throw new Error(error)
    }
})
...
router.get("/:username/recipes", authenticate, async (req,res)=>{
    try{
        ...
        res.status(500).json({
            errorMessage: 
            "sorry something went wrong getting user recipes"
        })
        throw new Error(error);
    }
})
...
router.put("/:id", authenticate, async (req,res)=>{
    try{
        ...
        res.status(500).json({
            errorMessage:
            `Sorry something went wrong updating your account`
        })
        throw new Error(error)
    }
})
...
router.delete("/:id", authenticate, async (req,res)=>{
    try{
        ...
        res.status(500).json({
            errorMessage:
            "Sorry something went wrong deleting your account"
        })
        throw new Error(error)
    }
})
