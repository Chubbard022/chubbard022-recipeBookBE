...
router.post(`/newrecipe`, authenticate, async (req, res) => {
  try {
    ...
    res.status(400).json({
      errorMessage: 'Sorry something went wrong adding recipe to insperation',
    });
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    res.status(400).json({
      errorMessage: 'Sorry something went wrong with udating recipe',
    });
    throw new Error(error);
  }
});
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    ...
    res.status(400).json({
      errorMessage: 'Sorry something went wrong deleting recipe',
    });
    throw new Error(error);
  }
});
