...
async function findByName(itemName) {
  return await db('pantryItems').where({ nameOfItem: itemName }).first();
}

//adds item to pantry list
async function add(newItem) {
  await db('pantryItems').insert(newItem);
...
async function update(id, updatedPantryItem) {
  return await db('pantryItems').where({ id }).update(updatedPantryItem);
}
// removes item from list of pantry items
async function remove(id) {
  return await db('pantryItems').where({ id }).del();
}
