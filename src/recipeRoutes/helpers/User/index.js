...
async function findUserByName(username) {
  return await db('users').where({ username }).select('id', 'username').first();
}
...
async function userRecipes(username) {
  return await db('users').join('recipes', 'recipes.username', '=', 'users.username').where({ 'recipes.username': username });
}
