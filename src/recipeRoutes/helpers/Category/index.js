...
async function findByName(categroyName) {
  return await db('categories').where({ name: categroyName }).first();
}
...
async function findAllRecipesInCategory(categoryName) {
  return await db('categories').join('recipes', 'recipes.category_name', '=', 'categories.name').where({ 'categories.name': categoryName });
}

//adds a new category to list
async function add(newCategory) {
  return await db('categories').insert(newCategory);
...
async function update(categoryID, updatedCategory) {
  await db('categories').where({ id: categoryID }).update(updatedCategory);
