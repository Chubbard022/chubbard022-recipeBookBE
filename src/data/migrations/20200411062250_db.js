exports.up = function (knex) {
  return knex.schema
    ...
    .createTable('recipeOfPantryItems', (itm) => {
      itm.increments('id');
      itm.integer('recipe_id').unsigned().references('recipe_id').inTable('recipes').onDelete('CASCADE').onUpdate('CASCADE');
      itm.integer('pantryItem_id').unsigned().references('pantry_id').inTable('pantryItems').onDelete('CASCADE').onUpdate('CASCADE');
    });
};
